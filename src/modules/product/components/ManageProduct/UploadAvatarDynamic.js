...
const UploadAvatarDynamic = (record) => {
  ...
  return (
    <ImgCrop beforeCrop={beforeUpload} rotate>
      <Upload
        action={"http://127.0.0.1:4000/" + "api/products/avatar/" + record.record.Id}
        ...
        onPreview={onPreview}
      >
        {fileList.length < 1 && (<Avatar shape="square" size={150} style={{ padding: 0 }} src={"http://127.0.0.1:4000/api/products/img/" + String(imageUrl)} />)}
      </Upload>
    </ImgCrop>
  );
};
